...
- **Preview deploys** are created for pull requests and non-`main` branches when enabled in the Vercel project (Settings → Git → Preview Deployments). If you only see Production builds, you are likely pushing directly to `main` with no open PRs — that is expected. Buyer demo sign-in on a Production-only workflow requires `ALLOW_DEMO_PREVIEW_ON_PRODUCTION=1` in addition to the preview demo vars (see [phase1-production-checklist.md](./phase1-production-checklist.md)).
