...
export type SeparationTaskPayload = {
  id: string;
  title: string;
...
export async function getMySeparationTasks(auth: AuthContext): Promise<SeparationTaskPayload[]> {
  ...
  return withAuthorizedTransaction(
    ...
    async (tx) => {
      ...
      const rows = await tx.employeeSeparationTask.findMany({
        where: { employeeId },
...
export async function patchMySeparationTask(
  auth: AuthContext,
  input: { taskId: string; status: OnboardingStatus },
): Promise<SeparationTaskPayload> {
  const employeeId = auth.subjectEmployeeId;
  if (!employeeId) {
    throw new ApiError(403, {
      code: "forbidden",
  ...
  return withAuthorizedTransaction(
    ...
    async (tx) => {
      const existing = await tx.employeeSeparationTask.findFirst({
      ...
      if (!existing) {
        throw new ApiError(404, {
          code: "not_found",
          message: "separation_task_not_found",
        });
      }
      ...
      if (nextIdx === -1 || nextIdx < curIdx) {
        throw new ApiError(400, {
          code: "bad_request",
          message: "separation_invalid_status_transition",
        });
      }
      ...
      const row =
        nextIdx === curIdx
          ? existing
          : await tx.employeeSeparationTask.update({
