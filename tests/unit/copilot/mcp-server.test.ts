...
describe("copilot MCP server scaffold", () => {
  it("responds to tools/list over stdio with Zod-derived schemas", () => {
  ...
  it("denies tools/call without COPILOT_MCP_ALLOW_STDIO", () => {
    const r = rpc({
      jsonrpc: "2.0",
      id: 2,
      method: "tools/call",
  ...
  it("rejects invalid arguments when stdio gate is open", () => {
    const r = rpc(
      {
        jsonrpc: "2.0",
        id: 3,
  ...
  it("accepts valid arguments when stdio gate is open", () => {
    const r = rpc(
      {
        jsonrpc: "2.0",
        id: 4,
