...
test.describe("Feature 008 manager team attendance today", () => {
  test("home → Team attendance shows roster or empty-direct-reports card", async ({
    page,
  }) => {
    const jwt = process.env.HR_ERP_MANAGER_TEAM_E2E_JWT?.trim();
    test.skip(
      !jwt,
      "Set HR_ERP_MANAGER_TEAM_E2E_JWT (manager role + demo manager subject id)",
    );
    ...
    await page.goto("/manager/team-attendance");
    await expect(
      page.getByRole("heading", { name: /^Team attendance today$/ }),
    ).toBeVisible({
      timeout: 30_000,
    });
    await expect(page.getByText(/Loading team attendance/)).toBeHidden({
      timeout: 30_000,
    });
    await expect(
      page
