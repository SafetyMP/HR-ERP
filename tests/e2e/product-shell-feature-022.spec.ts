...
test.describe("Feature 022 product shell", () => {
  test("UAC-1 employee paystub shows sidebar navigation", async ({ page }) => {
    ...
    await expect(
      page.getByRole("navigation", { name: /employee/i }),
    ).toBeVisible();
    await expect(page.getByRole("link", { name: "Paystub" })).toBeVisible();
    await expect(
      page.getByRole("link", { name: "Time & attendance" }),
    ).toBeVisible();
  ...
  test("UAC-2 manager shell shows team and recruiting nav", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("navigation", { name: /manager/i }),
    ).toBeVisible();
    await expect(
      page.getByRole("link", { name: "Team attendance" }),
    ).toBeVisible();
  ...
  test("UAC-3 HR dashboard shows sidebar navigation", async ({ page }) => {
    ...
    await expect(
      page.getByRole("navigation", { name: /HR operations/i }),
    ).toBeVisible();
    await expect(page.getByRole("link", { name: "Dashboard" })).toBeVisible();
    await expect(page.getByRole("link", { name: "Pay runs" })).toBeVisible();
  });

  test("UAC-4 anonymous home shows demo entry points not authenticated hub", async ({
    page,
  }) => {
    await page.goto("/");
  ...
  test("UAC-7 employee paystub avoids legacy zinc utility classes", async ({
    page,
  }) => {
    const jwt = process.env.HR_ERP_BENEFITS_E2E_JWT?.trim();
    test.skip(!jwt, "Set HR_ERP_BENEFITS_E2E_JWT");
    ...
    await page.goto("/employee/paystub");
    const hasZinc = await page
      .locator("#main-content [class*='zinc-']")
      .count();
    expect(hasZinc).toBe(0);
  });

  test("UAC-8 paystub reachable in two clicks from authenticated home", async ({
    page,
  }) => {
    const jwt = process.env.HR_ERP_BENEFITS_E2E_JWT?.trim();
    test.skip(!jwt, "Set HR_ERP_BENEFITS_E2E_JWT");
